...
- 1. Написать оболочку до unbound которая будет ходить в redis и параллельно в Unbound
- 2. Python код встроенный в официальный от Unbound pythonmod
...
То есть:
- cachedb сначала проверяет Redis;
- если в Redis есть запись — Unbound использует ее;
- если нет — Unbound делает обычную рекурсию;
...
Что реализовано                                                                                                      
  ...
  │           Файл           │                   Изменение                    │                                        
  ├──────────────────────────┼────────────────────────────────────────────────┤                                        
  │ unbound/local-zones.conf │ новый — подставная зона yandex.ru              │                                      
  ├──────────────────────────┼────────────────────────────────────────────────┤                                        
  │ unbound/unbound.conf     │ добавлен include: local-zones.conf (строка 37) │                                        
  ├──────────────────────────┼────────────────────────────────────────────────┤                                        
  │ unbound/Dockerfile       │ добавлен COPY local-zones.conf                 │                                        
  ├──────────────────────────┼────────────────────────────────────────────────┤                                        
  │ scripts/collect_zone.py  │ новый — сбор реальных записей + генерация conf │                                      
  └──────────────────────────┴────────────────────────────────────────────────┘                                        

  После исследования стандартного кэширования резолвера и подключения внешнего кэша следующим этапом проекта стала демонстрация локального переопределения зоны, подписанной DNSSEC, на уровне собственного DNS-резолвера.

  **Общая идея эксперимента**
  DNSSEC защищает публичную DNS-иерархию с помощью проверки цифровых подписей и цепочки доверия от корневой зоны к конечной записи. Однако при использовании собственного DNS-резолвера администратор может определить локальную зону и заставить резолвер отвечать по ней не через внешние авторитетные серверы, а из собственной локальной конфигурации.

  В рамках данного эксперимента не производится взлом DNSSEC. Демонстрируется именно то, что собственный резолвер может изменить источник DNS-ответа дляприкладного ПО пользователя, используя локально заданную зону.

Для демонстрации была выбрана зона cloudflare.com, которая подписана DNSSEC.
Факт использования DNSSEC подтверждался следующими командами:
```bash
dig @8.8.8.8 cloudflare.com DS +dnssec
dig @8.8.8.8 cloudflare.com DNSKEY +dnssec
dig @8.8.8.8 cloudflare.com A +dnssec
```
![Скриншот7](assets/terminal_screenshot_7.png)

В ответах наблюдались записи DS, DNSKEY и RRSIG, что подтверждает криптографическую защиту зоны cloudflare.com и корректность выбора этой зоны для выполнения задания.

## 3.1 Выбрать доменную зону с DNSSEC и разместить ее локально на резолвере

Для выполнения данного пункта в конфигурации Unbound была создана локальная зона для cloudflare.com. Локальная зона была вынесена в отдельный конфигурационный файл - `unbound/local-zones.conf`, и затем подключена к основному файлу конфигурации резолвера. Использовалось следующее локальное описание зоны:
```txt
server:
    local-zone: "cloudflare.com." static
    local-data: "cloudflare.com. 300 IN A 192.0.2.1"
    local-data: "www.cloudflare.com. 300 IN A 192.0.2.2"
```

Для демонстрации использовались адреса из диапазона 192.0.2.0/24, зарезервированного для документации и тестирования в RFC 5737. Это позволило однозначно отличать локально подставленные ответы от реальных публичных IP-адресов Cloudflare.

После изменения конфигурации контейнер с Unbound был пересобран и перезапущен:
...
```bash
dig @127.0.0.1 -p 2053 cloudflare.com A +short
dig @127.0.0.1 -p 2053 www.cloudflare.com A +short
```
![Скриншот7_1](assets/terminal_screenshot_7_1.png)
Результат:
- cloudflare.com возвращал 192.0.2.1;
- www.cloudflare.com возвращал 192.0.2.2.
Это подтверждает, что DNSSEC-подписанная зона была успешно размещена локально на собственном резолвере.

## 3.2 Настроить резолвер на ответы по этой зоне не от авторитетных серверов зоны, а из локальной зоны резолвера
Для того чтобы резолвер не обращался к внешним авторитетным серверам зоны, использовалась директива:
```conf
local-zone: "cloudflare.com." static
```
Тип static означает, что все запросы внутри зоны cloudflare.com обрабатываются локально и не передаются в обычный механизм рекурсивного разрешения.
Для проверки сравнивались ответы публичного DNS и собственного резолвера.
...
Локальный ответ собственного резолвера
```bash
dig @127.0.0.1 -p 2053 cloudflare.com A +short
```

![Скриншот8](assets/terminal_screenshot_8.png)

В результате публичный DNS возвращал реальные адреса Cloudflare, тогда как собственный резолвер возвращал тестовый адрес 192.0.2.1.

Это показывает, что резолвер был настроен на выдачу ответов по данной зоне не через внешние авторитетные DNS-серверы, а из локальной зоны.

## 3.3 Организовать сбор данных по выбранной зоне, закэшировать ответы и создать локальное описание зоны для резолвера (10 баллов, можно реализовать на своем собственном домене для уверенности в результатах)
Для подготовки локального описания зоны был реализован вспомогательный скрипт - `collect_zone.py`
Назначение скрипта:
- проверить наличие DNSSEC у выбранной зоны;
- собрать реальные DNS-ответы по зоне cloudflare.com;
- сформировать локальную конфигурацию для Unbound.
...
![Скриншот9_1](assets/terminal_screenshot_9_1.png)
![Скриншот9_2](assets/terminal_screenshot_9_2.png)

При выполнении скрипта:
- выполнялась проверка наличия DS, DNSKEY и RRSIG;
- собирались реальные DNS-записи для выбранных имен внутри зоны;
- на основе собранных данных генерировался файл local-zones.conf.
- Для большей наглядности в итоговом локальном описании реальные IP-адреса были заменены тестовыми адресами из диапазона 192.0.2.0/24. Это позволило одновременно:

- показать сбор данных по зоне;
- создать локальную конфигурацию зоны;
- сделать факт подмены наглядным и легко проверяемым.
- Таким образом, для резолвера была создана собственная локальная база данных зоны cloudflare.com, используемая вместо внешней публичной DNS-инфраструктуры.

## 3.4 Продемонстрировать, что резолвер отвечает информацией из своей базы данных, а не информацией с авторитетных серверов зоны (5 баллов, желательно с трассировкой рекурсивного поиска информации в системе DNS)
Для доказательства того, что резолвер отвечает из собственной локальной базы данных, использовались несколько способов проверки.

1. Сравнение публичного и локального ответа
```bash
dig @8.8.8.8 cloudflare.com A +short
dig @127.0.0.1 -p 2053 cloudflare.com A +short
```
![Скриншот9_3](assets/terminal_screenshot_9_3.png)
Публичный DNS возвращал реальные IP-адреса Cloudflare, а локальный резолвер — тестовый адрес 192.0.2.1.
Это уже указывало на использование локального описания зоны.

2. Проверка поведения внутри локальной зоны
```bash
dig @127.0.0.1 -p 2053 hack.cloudflare.com A
```
![Скриншот9_4](assets/terminal_screenshot_9_4.png)
Поскольку зона была определена как static, резолвер не пытался разрешать это имя через внешние серверы, а обрабатывал запрос локально в рамках своей конфигурации.

3. Анализ сетевого трафика
Для проверки отсутствия внешнего DNS-разрешения использовался tcpdump внутри контейнера Unbound:
```bash
docker exec -it dns-unbound tcpdump -tttt -n -i any '(udp port 53 or tcp port 53)'
```

А затем во второй вкладке терминала
```bash
dig @127.0.0.1 -p 2053 cloudflare.com A
```
![Скриншот9_5](assets/terminal_screenshot_9_5.png)

![Скриншот9_6](assets/terminal_screenshot_9_6.png)

В дампе трафика наблюдались:
- входящий запрос клиента к резолверу;
- исходящий ответ резолвера клиенту;
- но не наблюдалось новых внешних DNS-запросов для cloudflare.com.

Это подтверждает, что ответ формировался локально, без обращения к авторитетным серверам зоны.

4. Анализ логов резолвера
Дополнительно проверялись логи Unbound:
```bash
docker compose logs unbound | grep -i cloudflare
```

![Скриншот9_7](assets/terminal_screenshot_9_7.png)
